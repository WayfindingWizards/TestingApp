...
function useBLE(): BluetoothLowEnergyApi {
  ...
  const [location, setLocation] = useState<[number, number]>([-1, -1]);  // ***Question: Can we even have default values? How else do we do this?
    ...
    bleManager.startDeviceScan(
      ...
      (error, device) => {
        if (device?.name?.includes('IPSWW')) { // find beacon with general identifier
          ...
          const deviceBuffer = new Queue<[number, number, number]>; // [rssi, xCoord, yCoord]
          const queueSize = 20; //
          ...
          let orderedBuffer = Array<[number, number, number]>;
          let devicePackage: [number, number, number] | undefined;
          ...
          for (let i = 0; i < deviceBuffer.size(); i++){
            if (devicePackage !== undefined && deviceBuffer !== undefined) {
              ...
              orderedBuffer.prototype.push(devicePackage);
              if (devicePackage !== undefined && deviceBuffer !== undefined) {
              deviceBuffer.enqueue(devicePackage);}
            }
          }

          function trilaterate(beacon1: any[], beacon2: any[], beacon3: any[]) {
          ...
          orderedBuffer.prototype.sort((a, b) => a[0] - b[0]); // order by descending distance/RSSI
          if (orderedBuffer.length >= 3){
            let userCoords: [number, number] = trilaterate(orderedBuffer[0], orderedBuffer[1], orderedBuffer[2]);
